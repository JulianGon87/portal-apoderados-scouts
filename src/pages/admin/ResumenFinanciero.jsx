import React from 'react';
import AdminLayout from '../../components/admin/AdminLayout';

const ResumenFinanciero = () => {
    return (
        <AdminLayout title="Resumen Financiero">
            <div className="bg-white p-6 rounded-lg shadow-md">
                <div className="text-center py-12">
                    <span className="text-6xl mb-4 block">📈</span>
                    <h2 className="text-2xl font-bold text-gray-800 mb-2">Resumen Financiero</h2>
                    <div className="bg-yellow-50 border-l-4 border-yellow-400 p-4 mb-6 inline-block text-left">
                        <div className="flex">
                            <div className="flex-shrink-0">
                                <svg className="h-5 w-5 text-yellow-400" viewBox="0 0 20 20" fill="currentColor">
                                    <path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd" />
                                </svg>
                            </div>
                            <div className="ml-3">
                                <p className="text-sm text-yellow-700">
                                    <span className="font-bold">En Construcción:</span> Estamos trabajando para traerte el mejor resumen financiero.
                                </p>
                            </div>
                        </div>
                    </div>
                    <p className="text-gray-600 mb-6">Próximamente podrás ver los movimientos y el estado financiero detallado.</p>
                    <div className="animate-pulse bg-gray-200 h-64 w-full rounded-lg"></div>
                </div>
            </div>
        </AdminLayout>
    );
};

export default ResumenFinanciero;
